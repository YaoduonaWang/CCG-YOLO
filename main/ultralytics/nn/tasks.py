...
from ultralytics.nn.attention import CBAM, GAM_Attention
import torch
...
class DetectionModel(BaseModel):
    ...
    def __init__(self, cfg="yolov8n.yaml", ch=5, nc=None, verbose=True):  # model, input channels, number of classes
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in {
            ...
            if m in {
                ...
                n = 1
            if m is {C3k2, C3k2_AP}:  # for M/L/X sizes
                legacy = False
                if scale in "mlx":
                    args[3] = True
        elif m is AIFI:
            ...
            if m is HGBlock:
                ...
                n = 1
        elif m in {CAFM, SegNext_Attention, DAttention, MLCA, SegNext_Attention,
                    LocalWindowAttention, BiLevelRoutingAttention, BiLevelRoutingAttention_nchw,
                   CPCA, AFGCAttention, EfficientChannelAttention, MPCA}:
            c2 = ch[f]
            args = [c2, *args]
        elif m in {SimAM}:
            c2 = ch[f]

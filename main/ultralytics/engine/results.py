...
from ultralytics.utils.multispectral import extract_bgr
...
class Results(SimpleClass):
    ...
    def __init__(
        ...
        self.bgr_image = extract_bgr(self.orig_img) 
        self.orig_shape = self.bgr_image.shape[:2]
        self.boxes = Boxes(boxes, self.bgr_image) if boxes is not None else None  # native size boxes
        self.masks = Masks(masks, self.bgr_image) if masks is not None else None  # native size or imgsz masks
    ...
    def new(self):
        ...
        """
        return Results(orig_img=self.bgr_image, path=self.path, names=self.names, speed=self.speed)
    ...
    def plot(
        ...
        if img is None and isinstance(self.bgr_image, torch.Tensor):
            img = (self.bgr_image[0].detach().permute(1, 2, 0).contiguous() * 255).to(torch.uint8).cpu().numpy()
        ...
        annotator = Annotator(
            deepcopy(self.bgr_image if img is None else img),
            line_width,
            font_size,
            font,

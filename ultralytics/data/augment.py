...
class LetterBox:
    ...
    def __call__(self, labels=None, image=None):
        ...
        default_border = 114
        border_value = (default_border,) * num_channels

        img = self._add_border(
    ...
    def _add_border(self, img, top, bottom, left, right, value=114):
        """
        Custom border adding to enable multispectral imaging
        """

        num_channels = img.shape[-1]
        border_value = (value,) * num_channels

        h, w = img.shape[:2]
        new_shape = (h + top + bottom, w + left + right, img.shape[-1])
        bordered_img = np.full(new_shape, border_value, dtype=img.dtype)

        # Place the original image in the centre
        bordered_img[top:top + h, left:left + w] = img
        return bordered_img
...
def v8_transforms(dataset, imgsz, hyp, stretch=False):
    ...
    return Compose(
        [
            ...
            #RandomHSV(hgain=hyp.hsv_h, sgain=hyp.hsv_s, vgain=hyp.hsv_v), # Disabled for multispectral YOLO

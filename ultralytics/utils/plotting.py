...
def plot_images(
    ...
    for i in range(bs):
        ...
        mosaic[y : y + h, x : x + w, :] = images[i][:3, :, :].transpose(1, 2, 0) # Plot only RGB channels
    ...
    for i in range(bs):
        ...
        if paths:
            annotator.text((x + 5, y + 5), text=Path(paths[i][0]).name[:40], txt_color=(220, 220, 220))  # filenames
        if len(cls) > 0:
            idx = batch_idx == i
